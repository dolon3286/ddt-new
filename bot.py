...
def render_status(name: str, snapshot: TaskStatusSnapshot, task_id: str) -> str:
    percent = (
        (snapshot.processed_bytes / snapshot.total_bytes) * 100.0
        if snapshot.total_bytes > 0
        else 0.0
    )
    processed = format_bytes(snapshot.processed_bytes)
    total = format_bytes(snapshot.total_bytes) if snapshot.total_bytes > 0 else "?"
    current = snapshot.current_name or name
    if snapshot.stage.lower().startswith("download"):
        header = f"Downloading: {current}"
    elif snapshot.stage.lower().startswith("upload"):
        header = f"Uploading: {current}"
    else:
        header = f"{snapshot.stage}: {current}"
    return (
        f"{header}\n"
        f"{processed} of {total} ({percent:.2f}%)\n"
        f"Stop: /c{task_id}"
    )
...
def upload_files_to_drive(
    ...
    for file_path in files:
        ...
        try:
            ...
            if cancel_event.is_set():
                break
            uploaded.append(f"{file_path} -> {response.get('id')}")
...
async def run_download_and_upload(
    ...
    if process.returncode != 0:
        status.update_progress(stage="Download failed")
        await safe_edit_text(status_message, render_status(link, status.snapshot(), task_control.task_id))
        return DownloadResult([], [])
    ...
    try:
        ...
        status.update_progress(stage="Upload failed")
        await safe_edit_text(status_message, render_status(link, status.snapshot(), task_control.task_id))
        return DownloadResult([], [])
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.COMMAND, cancel_task))
